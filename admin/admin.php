...
<body>
    ...
    <div>
        <form action="" method="POST">
            ...
            <button style="width: 5em; padding: 0.7em;color: white;border: none;border-radius: 0 10px 10px 0;background: linear-gradient(222.28deg, #7000FF, rgba(250, 0, 255) 98.17%);" name="btnSerchAdmin" class='btnSerchAdmin'><i class="fa-solid fa-magnifying-glass"></i></button>
        </form>
    </div>
<div class="mb-3">
...
if($result->rowCount() > 0){
    while($row = $result->fetch()) {
        ...
        $Reservation_Expiration_Date = $row['Reservation_Expiration_Date'];
        ...
        if($Reservation_Expiration_Date <= $date){
...
<div class="cart mt-5 d-flex  justify-content-between">
        ...
        <div class="d-flex flex-column justify-content-between">
            ...
            <div class="mt-5 ms-3">
                <form action="" method="POST">
                    <button type="submit" name="btnAccept" value="<?php echo $reservCode ?>"  class="btn btn-dark border-white border-3 me-3">Acceptance</button>
                    <button type="submit" name="btnReject" value="<?php echo $reservCode ?>"  class="btn btn-outline-dark bg-white border-white border-3 reject">to reject</button>
                </form>
            </div>
        </div>
...
if(isset($_POST["btnAccept"])){
    $reservCode = $_POST["btnAccept"];
    $select ="SELECT * FROM `reservation` WHERE `Reservation_Code` = $reservCode";
    $result = $db->query($select);
    while($row = $result->fetch()) {
        $itemCode = $row['Item_Code'];
        $Nickname = $row['Nickname'];
    ...
    $sql = "UPDATE `item` SET `Status`='Borrowed' WHERE `Item_Code` = '$itemCode'";
    $db->query($sql);
